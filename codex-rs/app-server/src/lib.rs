...
pub async fn run_main_with_transport_options(
    ...
    // Create the config loader; loading and I/O happen below.
    ...
    // First load: discover enough configuration to install the cloud config loader.
    ...
    // Second load: assemble the complete configuration with cloud layers available.
    ...
    // Initialize the SQLite-backed application state before request processing starts.
    ...
    let processor_handle = tokio::spawn({
        ...
        // MessageProcessor is the central JSON-RPC dispatcher. The surrounding task owns
        // connection lifecycle, initialization gating, and shutdown behavior.
