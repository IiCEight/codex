...
async fn run_compact_task_inner(
    ...
    // Perform the compaction after the lifecycle hooks have admitted it.
...
async fn run_compact_task_inner_impl(
    ...
    // Work on a snapshot of the live history and append the compaction prompt only to it.
    ...
    // The live history now contains the preceding conversation and the generated summary,
    // but not the temporary compaction prompt used above.
    ...
    // Preserve recent user input alongside the summary in the replacement history.
    let user_messages = collect_annotated_user_messages(history_items);

    // Keep the newest user messages up to COMPACT_USER_MESSAGE_MAX_TOKENS, oldest first,
    ...
    // Atomically replace the live history and its persisted compaction checkpoint.
