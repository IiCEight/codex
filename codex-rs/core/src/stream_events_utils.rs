...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Ok(Some(call)) => {
            // Open mailbox delivery before spawning the tool so other agents can steer this turn
            ...
            // Persist the call before execution, including when execution is later cancelled.
            record_completed_response_item(ctx.sess.as_ref(), ctx.step_context.as_ref(), &item)
                .await;

            let cancellation_token = ctx.cancellation_token.child_token();
            // handle_tool_call is the point where the tool enters the runtime execution path.
