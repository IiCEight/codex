...
pub(crate) async fn run_turn(
    ...
    // Compact before sampling when the current context has reached its budget.
    ...
    // Discover the skill and plugin context that should be visible during this turn.
    ...
    // Record user input in ContextManager's conversation history after hooks admit it.
    ...
    loop {
        ...
        // Steering input is appended to the same conversation history before the next sample.
        if run_hooks_and_record_inputs(
            &sess,
            &turn_context,
            &turn_context.capture_current_model_info(),
...
async fn run_sampling_request(
    ...
    // Tool calls in this sampling round share one runtime and parallel-execution gate.
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        match event {
            ...
            ResponseEvent::OutputItemDone(mut item) => {
                ...
                // Completed model output is where tool calls enter the execution pipeline.
                let output_result =
